...
def kitti_flow_read(fpath):
    ...
    optical_flow = np.dstack((u, v, valid))
    #return u,v,valid
    return optical_flow
...
def kitti_disp_write(d, fpath):
    ...
    I = I.astype('uint16')
    plane_count, row_count, column_count  = I.shape

    pngfile = open(fpath, 'wb')

    try:
        W = png.Writer(width=column_count,
                       height=row_count,
                       greyscale=True,
                       bitdepth=16)

        W.write(pngfile, np.reshape(I, (-1, column_count*plane_count)))
